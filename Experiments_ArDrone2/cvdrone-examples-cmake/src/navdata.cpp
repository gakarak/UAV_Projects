...
double ARDrone::getACC(double *ax, double *ay, double *az)
{
    if (mutexNavdata) pthread_mutex_lock(mutexNavdata);
    double acc_x =  navdata.raw_measures.raw_accs[0];
    double acc_y =  navdata.raw_measures.raw_accs[1];
    double acc_z =  navdata.raw_measures.raw_accs[2];
...
double ARDrone::getTime()
{
    if (mutexNavdata) pthread_mutex_lock(mutexNavdata);
    int timeDrone =  navdata.time.time;
    if (mutexNavdata) pthread_mutex_unlock(mutexNavdata);
    return timeDrone;
}
