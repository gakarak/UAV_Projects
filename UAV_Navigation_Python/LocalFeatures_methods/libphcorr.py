...
def imregcorr(img1,img2, isUseGradMag=False, isDebug=False, isSelectBestShiftPQ=False, pqThreshBestShift=0.1):
    ...
    if isDebug:
        cv2.imshow("win0-original", nrmMat(np.concatenate( (img1, img2 ), axis=1 )))
        cv2.imshow("win1-FFT-ABS",  nrmMat(np.concatenate( (np.log(img1F+1),np.log(img2F+1)), axis=1 )))
        cv2.imshow("win2-FFT-LPT",  nrmMat(np.concatenate( (img1FLP,img2FLP), axis=1 )))
        cv2.imshow("win3-CC-LogPolar",  nrmMat(CCRS))
        if isSelectBestShiftPQ:
            cv2.imshow("win4-CC-Shift-XY (CCS0,CCS)",  nrmMat(np.concatenate( (CCS0,CCS), axis=1 )))
        else:
            cv2.imshow("win4-CC-Shift-XY",  nrmMat(CCS))
        img2RS_Shifted=np.roll(np.roll(img2RS.copy(),int(np.round(dxyS[0])),1), int(np.round(dxyS[1])), 0)
        cv2.imshow("win5-Registered",  np.dstack( (nrmMat(img1),nrmMat(img2RS_Shifted),nrmMat(img1)) ))
...
def calcWeightedCenter(mat, pos, siz=3):
    ...
    h=mat.shape[0]
    isBorder=False
    if (pos[1]<siz) or ((pos[1]+siz)>=w):
        isBorder=True
    if (pos[0]<siz) or ((pos[0]+siz)>=h):
        isBorder=True
    if isBorder:
        return pos
    else:
    ...
    def read(self, parPos=-1):
        ...
        if parPos<0:
            ...
            if parPos<numfrm:
                ...
                return (False, None)
    def readWithName(self, parPos=-1):
        numfrm=self.getNumFrames()
        if parPos<0:
            if self.pos<numfrm:
                ret=(True, cv2.imread(self.listFImg[self.pos], -1), self.listFImg[self.pos])
                self.pos+=1
                return ret
            else:
            ...
            if parPos<numfrm:
                ret=(True, cv2.imread(self.listFImg[parPos], -1), self.listFImg[self.pos])
                return ret
            else:
                return (False, None, None)
    def getKPByID(self, idx, fpName):
        if len(self.listFImg)<1:
            return None
        if (idx<0) or (idx>=len(self.listFImg)):
            idx=0
            return None
        fimg=self.listFImg[idx]
        fkp='%s_%s.kp' % (fimg, fpName)
        if not os.path.isfile(fkp):
            return None
        return fkp
    def printInfo(self):
