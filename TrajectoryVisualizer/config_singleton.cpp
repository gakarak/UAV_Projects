...
void ConfigSingleton::loadIni(string path_to_ini)
{
  ...
  {
    path_to_trj1_csv = ini.value("Trajectory1/path_to_csv").toString().
                                                            toStdString();
  }

  if (ini.contains("Trajectory2/path_to_csv"))
  {
    path_to_trj2_csv = ini.value("Trajectory2/path_to_csv").toString().
                                                            toStdString();
  }
  ...
  {
    gradient_m_per_px = 4;
    clog << "Common meters per pixel for gradient missed in '" +
            path_to_ini + "'" << endl;
    clog << "Common meters per pixel for gradient default value " <<
            gradient_m_per_px << endl;
    //throw Exception("ini file doesn't contain the key:
    //                   Common/gradient_meters_per_pixel");
  }
...
string ConfigSingleton::getPathToDescriptors(int trj_num, string detector_name,
                                                          string descriptor_name)
{
  string path_to_trj = trj_num == 0? path_to_trj1_csv: path_to_trj2_csv;
  return path_to_trj + "_" + detector_name + "_" +
                             descriptor_name + "_descriptors.bin";
}

string ConfigSingleton::getPathToKDTree(int trj_num , string detector_name,
                                                      string descriptor_name)
{
  string path_to_trj = trj_num == 0? path_to_trj1_csv: path_to_trj2_csv;
  return path_to_trj + "_" + detector_name + "_" +
                             descriptor_name + "_KDTree.bin";
}
...
string ConfigSingleton::getPathToDescriptors(string path_to_trj_csv,
                                             ...
                                             string descriptor_name)
{
  return path_to_trj_csv + "_" + detector_name + "_" +
                                 descriptor_name + "_descriptors.xml";
}
