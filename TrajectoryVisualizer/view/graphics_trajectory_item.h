...
class GraphicsTrajectoryItem : public QObject, public QGraphicsItemGroup
{
 ...
 public:
  ...
  void pushBackFrame(QPixmap img, QPointF center_coords_px, double angle,
                     double meters_per_pixel, double quality);
  void addKeyPoint(int frame_num, QPointF center_px, double angle,
                   double radius, QColor color);
  void addKeyPointNew(QPointF pos, double angle, double radius,
                      double scale, QColor color);
 ...
 private:
  void makeTransforms(std::shared_ptr<QGraphicsItem> item,
                      QPointF item_center_px, QPointF scene_center_pos_px,
