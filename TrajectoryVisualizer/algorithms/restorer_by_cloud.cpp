...
double RestorerByCloud::recoverLocation(const cv::Rect2f &frame_rect,
                                        cv::Point2f &pos,
                                        double &angle, double &scale)
{
  ...
  std::cout << getMatcher()->getTrainDescriptors().size() << std::endl;
  ...
  if (!homography.empty())
  {
    ...
    cv::Point2f que_center = (frame_rect.tl() + frame_rect.br()) / 2.;
